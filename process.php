           ...
           // duplicate('dbs10633164',$company_name);
            ...
            {  
              die('Could not connect: ' . mysqli_error());  
                    ...
                    $getTables =  @mysqli_query($conn ,"SHOW TABLES") or die(mysqli_error($conn));   
                    $originalDBs = [];
                    while($row = mysqli_fetch_row( $getTables )) {
                            $originalDBs[] = $row[0];
                        }
                        
                    @mysqli_query($conn ,"CREATE DATABASE `$newDB`") or die(mysqli_error($conn));  
                    foreach( $originalDBs as $tab ) {
                            @mysqli_select_db ( $conn ,$newDB ) or die(mysqli_error($conn ));
                            @mysqli_query($conn ,"CREATE TABLE $tab LIKE ".$originalDB.".".$tab) or die(mysqli_error($conn));
                        @mysqli_query($conn ,"INSERT INTO $tab SELECT * FROM ".$originalDB.".".$tab) or die(mysqli_error($conn));
                    }
					mysqli_close($conn);
